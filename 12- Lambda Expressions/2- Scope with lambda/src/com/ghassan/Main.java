...
public class Main {
    ...
    //Through this example we discuss the variable scope in lambda
    //Important note: when we use a local variable inside the anonymous class block it
    //should be final, because the variable will be used at the moment that the instance of
    //anonymous class is created, so it should not be changed if we try no to do that we will get
    //compiler error
    //the same case is applied for lambda, we should use local variables which are final or
    //effectively final which means that does not change its value
    public static void main(String[] args) {
        ...
        System.out.println("==============================================================");
        //Here the for loop will not work because the employee.get(i) is not effectively final
        //We can solve that by creating a varinbl on each iteration or by using the enhanced for loop
...
class AnotherClass{
    public String doSomething(){
        //This variable should be effectively final since we use it inside lambda
        int i = 0;
