These examples illustrate the basic input/output in Java
The follwoing concepts are implemented:
1- Exceptions
2- Try with resources 
3- Character streams(FileReader and FileWriter)
4- Byte streams (DataInputStream and DataOutputStream)
5- The BufferedReader/BufferedWriter and the BufferedInputStream/BufferedOutputStream
6- Reading and writing Objects (Serialization) with ObjectInputStream and ObjectOutpuStream
7- Random access file
